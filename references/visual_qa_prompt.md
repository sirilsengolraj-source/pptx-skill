...
density, empty-ratio). It cannot see composition problems that only
surface visually: line-through-text collisions, wrapped-title decoration
mismatches, uneven gaps, low contrast, or a slide that is technically
correct but reads as empty. Those need fresh eyes.
...
2. **List issues found.** If none reported, ask again more critically.
3. Fix issues at the source (outline or renderer), not in the mutated
   PPTX.
4. **Re-verify affected slides** — one fix often creates another
