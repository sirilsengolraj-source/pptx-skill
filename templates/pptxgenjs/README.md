...
| `slides.js`  | One function per slide family, plus the shared chrome (dark title bar, footer, notes). Exports `renderTitle`, `renderSection`, `renderStandard`, `renderCards`, `renderSplit`, `renderTimeline`, `renderStats`, and canvas constants (`SLIDE_W`, `SLIDE_H`, `MARGIN_X`, `HEADER_TOP`, `TITLE_BAR_H`, `CONTENT_TOP`). |
...
| `content / cards-2`            | `renderCards`     | Two equal-width cards with top accent rail.               |
| `content / cards-3`            | `renderCards`     | Three equal-width cards.                                  |
| `content / split`              | `renderSplit`     | Bullets left, dark highlights panel right.                |
| `content / timeline`           | `renderTimeline`  | Horizontal rail with marker dots + per-milestone cards.   |
...
3. Every text box sets `margin: 0` so the baseline math matches the layout
   coordinates exactly.
4. Canvas is `10.0" x 5.625"` (16:9). Side margins 0.5", header top 0.3",
   dark title bar 0.90" tall. Content rail starts at `y = 1.20`.
