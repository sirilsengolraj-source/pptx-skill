        ...
        "slides": [
            {
                "type": "title",
                "title": title,
                "subtitle": "Update the workspace notes, assets, and outline before first build",
            },
            {
                "type": "content",
                "variant": "split",
                ...
                "bullets": [
                    ...
                    "Break dense material into separate slides when needed.",
                ],
                "highlights": [
            ...
            },
            {
                "type": "content",
                "variant": "flow",
                "title": "Durable authoring loop",
                "subtitle": "Plan before rendering, then iterate from saved source",
                "footer": "Native Mermaid fallback renders this diagram without the retired bridge path.",
                "assets": {
                    "mermaid_source": "assets/diagrams/authoring_loop.mmd"
                },
            },
        ],
    }
...
2. Fill `evidence_plan.json` with sourced claims, metrics, and chart candidates.
3. Update `notes.md` with data rules and unresolved assumptions.
4. Add source-backed image/background/chart requests to `asset_plan.json`.
5. Stage local assets inside `assets/` when needed.
6. Edit `outline.json` to add, replace, or reorder slides.
7. Reference staged assets with aliases such as `asset:hero_name`, `image:crew_portrait`, or `generated:concept_visual`.
8. Re-run `build_workspace.py`.
9. Keep the source files. Do not rely on inline heredoc generation if you want to extend the deck later.
"""
...
def _content_plan_stub(title: str) -> dict[str, Any]:
    return {
        ...
        "narrative_arc": [
            ...
            {
                "act": "workflow",
                "purpose": "Show how source-first authoring keeps later edits clean.",
                "slides": ["s2", "s3"],
            },
            {
                "act": "implication",
        ...
        "slide_plan": [
            ...
            },
            {
                "slide_id": "s3",
                "role": "workflow",
                "message": "Plan, stage, render, and QA in a repeatable loop.",
                "variant": "flow",
                "visual_strategy": "native Mermaid diagram rendered to PNG at build time",
                "evidence_needs": [],
                "asset_needs": ["assets/diagrams/authoring_loop.mmd"],
            },
        ],
        "design_notes": {
            "style_preset_reason": "Starter uses the requested preset while keeping typography and spacing conservative.",
            "rhythm_break": "Slide 3 uses a diagram so the scaffold is not text-only.",
            "visual_motif": "Workflow loop with clear stage labels.",
        },
    }
...
def _starter_mermaid_source() -> str:
    return """flowchart LR
  A[Content plan] --> B[Evidence plan]
  B --> C[Asset staging]
  C --> D[Render PPTX]
  D --> E[QA gate]
  E --> B
...
def main() -> int:
    ...
    _write_text(workspace / "assets" / "diagrams" / "authoring_loop.mmd", _starter_mermaid_source())
